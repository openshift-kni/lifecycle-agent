...
- **Stage transitions must be valid**: controller computes `status.validNextStages`; changing `spec.stage` to anything else is rejected as an invalid transition.
...
- **Idle**
  ...
  - Runs health checks (unless skipped).
...
- **Config**
  - Executes the configuration flow:
    ...
    - **post-pivot**: edits the node and cluster, wait for cluster to stabilize and verify `status` matches `spec`
...
- **`lca.openshift.io/ipconfig-skip-cluster-health-checks`**: presence flag to skip cluster health checks (use carefully).
...
- **Health checks never pass**
  - Investigate cluster health blockers; as a last resort for lab testing, use the skip-health annotation:
    - `lca.openshift.io/ipconfig-skip-cluster-health-checks: ""` (presence flag)
