...
//   - the IPC CR exists but has no status conditions yet (not initialized), OR
//   - IPC spec.stage is Idle AND either:
//   - the IPC Idle condition is present and true, OR
//   - IPC is currently Blocked (by gating) and needs to be unblocked.
//
// Otherwise IBU gates reconciliation as follows:
//   - If the IPC CR does not exist yet: requeue soon (without updating IBU status).
//   - If IPC is not idle (stage != Idle or Idle condition is not true): mark IBU as Blocked and requeue after a short interval.
func (r *ImageBasedUpgradeReconciler) gateIBUByIPConfig(
	...
	// If the IPC CR exists and has status conditions and is not idle,
	// block the IBU.
