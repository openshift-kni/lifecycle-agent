...
func newFakeIPClient(t *testing.T, objs ...*ipcv1.IPConfig) *fake.ClientBuilder {
	...
	objsAny := make([]crclient.Object, 0, len(objs)+1)
	objsAny = append(objsAny, mc)
	for _, o := range objs {
...
func Test_validateIPSpecFamilies_ValidIPv4AndIPv6(t *testing.T) {
	ipc := &ipcv1.IPConfig{
		Spec: ipcv1.IPConfigSpec{
			IPv4: &ipcv1.IPFamilyConfig{
				Address:        "192.168.130.130",
				Gateway:        "192.168.130.1",
				DNSServer:      "192.168.130.1",
				MachineNetwork: "192.168.130.0/24",
			},
	...
	err := validateIPSpecFamilies(ipc)
	assert.NoError(t, err)
}

func Test_validateIPSpecFamilies_MismatchedFamiliesAndInvalidCIDR(t *testing.T) {
	ipc := &ipcv1.IPConfig{
		Spec: ipcv1.IPConfigSpec{
			IPv4: &ipcv1.IPFamilyConfig{
				Address:        "192.168.130.130",
				Gateway:        "2001:db8::1",   // IPv6 in IPv4 field
				DNSServer:      "not-an-ip",     // invalid
				MachineNetwork: "2001:db8::/64", // IPv6 CIDR in IPv4 field
			},
			IPv6: &ipcv1.IPFamilyConfig{
				Address:        "10.0.0.1",   // IPv4 in IPv6 field
				Gateway:        "10.0.0.254", // IPv4
				DNSServer:      "2001:db8::53",
				MachineNetwork: "invalid-cidr", // invalid
			},
		},
	}
	...
	assert.Error(t, err)
	msg := err.Error()
	assert.Contains(t, msg, "spec.ipv4.gateway")
	assert.Contains(t, msg, "spec.ipv4.dnsServer")
	assert.Contains(t, msg, "spec.ipv4.machineNetwork")
	assert.Contains(t, msg, "spec.ipv6.address")
	assert.Contains(t, msg, "spec.ipv6.machineNetwork")
}
