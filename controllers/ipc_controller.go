...
func readIPConfigStatus(filePath string, ops ops.Ops) (common.IPConfigStatusType, string, error) {
	data, err := ops.ReadFile(filePath)
	if err != nil {
		if ops.IsNotExist(err) {
			return common.IPConfigStatusUnknown, "", nil
		}
		return common.IPConfigStatusUnknown, "", fmt.Errorf("failed to read status file %s: %w", filePath, err)
	}

	var st common.IPConfigStatus
	if err := json.Unmarshal(data, &st); err != nil {
		return common.IPConfigStatusUnknown, "", fmt.Errorf("failed to parse status file %s: %w", filePath, err)
	}

	switch st.Phase {
	case common.IPConfigStatusRunning, common.IPConfigStatusSucceeded, common.IPConfigStatusFailed:
		return st.Phase, st.Message, nil
	default:
		return common.IPConfigStatusUnknown, st.Message, nil
	}
}
