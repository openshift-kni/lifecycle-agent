...
func (r *ImageBasedUpgradeReconciler) getSeedImage(
	...
	hasProxy, err := lcautils.HasProxy(ctx, r.Client)
	if err != nil {
		return fmt.Errorf("failed to check if cluster has proxy: %w", err)
	}

	r.Log.Info("Checking seed image proxy compatibility")
	if err := checkSeedImageProxyCompatibility(seedInfo.HasProxy, hasProxy); err != nil {
		return fmt.Errorf("checking seed image compatibility: %w", err)
	}
...
func getSeedConfigFromLabel(labels map[string]string) (*seedclusterinfo.SeedClusterInfo, error) {
	...
	if !ok {
		return nil, fmt.Errorf("seed image is missing the %s label, please build a new image using the latest version of LCA", common.SeedFormatOCILabel)
	}
...
// doesn't.
func checkSeedImageProxyCompatibility(seedHasProxy bool, hasProxy bool) error {
