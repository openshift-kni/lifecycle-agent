...
func CreateKubeClientFromBytes(scheme *runtime.Scheme, kubeconfigData []byte) (runtimeclient.Client, error) {
	...
	if err != nil {
		return nil, fmt.Errorf("failed to build rest config from kubeconfig bytes: %w", err)
	}
	config.WarningHandler = rest.NoWarnings{}
	rc, err := runtimeclient.New(config, runtimeclient.Options{Scheme: scheme})
	if err != nil {
...
func BuildKernelArgumentsFromMCOFile(path string) ([]string, error) {
	...
	return BuildKernelArgumentsFromMCOObject(mc)
}

func BuildKernelArgumentsFromMCOData(data []byte) ([]string, error) {
	mc := &mcfgv1.MachineConfig{}
	if err := k8syaml.Unmarshal(data, mc); err != nil {
