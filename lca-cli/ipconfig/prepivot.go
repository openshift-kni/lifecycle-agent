...
func (p *PrePivotHandler) setDefaultDeploymentIfEnabled(ctx context.Context, newStateroot string) error {
	if !p.ostree.IsOstreeAdminSetDefaultFeatureEnabled(ctx) {
...
// restoreMCDManagedVarLibFiles copies MCO-managed files under /var/lib/
// from the old stateroot to the new one. This is needed because
// removeStaleFilesInNewStaterootForRegeneration may remove directories
// containing MCO-managed files (e.g. enable_dynamic_cpu_affinity under
...
func (p *PrePivotHandler) restoreMCDManagedVarLibFiles(ctx context.Context) error {
	...
	for _, f := range managedFiles {
		srcPath := filepath.Join(p.ostreeData.OldStateroot.Path, f)
		dstPath := filepath.Join(p.ostreeData.NewStateroot.Path, f)

		p.log.Infof("Restoring MCO-managed file %s from old stateroot", f)

		if err := p.ops.MkdirAll(filepath.Dir(dstPath), os.FileMode(0o755)); err != nil {
