...
func (p *IPConfigPostPivotHandler) Run(ctx context.Context) error {
	...
	if err := utils.RunOnce("apply-network-configuration", p.workspaceDir, p.log, func() error {
		return p.applyNetworkConfiguration(ctx)
	}); err != nil {
		return fmt.Errorf("failed to run once apply-network-configuration: %w", err)
	}

	p.log.Info("Running recert")
	if err := utils.RunOnce("run-recert", p.workspaceDir, p.log, func() error {
		return p.runRecert(ctx)
	}); err != nil {
