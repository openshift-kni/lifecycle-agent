...
func (p *PostPivot) setDnsMasqConfiguration(seedReconfiguration *clusterconfig_api.SeedReconfiguration,
	...
	p.log.Infof("Setting new dnsmasq and forcedns dispatcher script configuration for %s", seedReconfiguration.NodeIPs[0])
	...
	config := []string{
		...
		fmt.Sprintf("SNO_DNSMASQ_IP_OVERRIDE=%s", seedReconfiguration.NodeIPs[0]),
	}
