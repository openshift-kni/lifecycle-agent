...
func readPrecacheSpecFile() (precacheSpec []string, err error) {
	...
	if _, err := os.Stat(precacheSpecFile); os.IsNotExist(err) { //nolint:gosec // precacheSpecFile path is validated
		return precacheSpec, fmt.Errorf("missing precache spec file")
	}
	log.Info("Precache spec file found.")

	var content []byte
	content, err = os.ReadFile(precacheSpecFile) //nolint:gosec // precacheSpecFile path is validated
	if err != nil {
		return
	}
