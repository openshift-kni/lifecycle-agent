...
func (c *Client) Deploy(ctx context.Context, osname, refsepc string, kargs []string, rpmOstreeClient rpmostreeclient.IClient, ibi bool) error {
	...
	if enabled, err := c.IsOstreeAdminSetDefaultFeatureEnabled(ctx); err != nil {
		return fmt.Errorf("failed to check ostree admin set-default feature: %w", err)
	} else if !c.ibi && enabled {
