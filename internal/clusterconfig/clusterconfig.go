...
const (
	...
	installConfigConfigmapNamespace = "kube-system"
	installConfigConfigmapName      = "cluster-config-v1"
	installConfigConfigmapDataKey   = "install-config"
...
func (r *UpgradeClusterConfigGather) GetInstallConfig(ctx context.Context) (string, error) {
	...
	if err := r.Client.Get(ctx, types.NamespacedName{Namespace: installConfigConfigmapNamespace, Name: installConfigConfigmapName}, &configmap); err != nil {
		return "", fmt.Errorf("failed to get install-config configmap: %w", err)
	}
	return configmap.Data[installConfigConfigmapDataKey], nil
