	...
	testcases := []struct {
	...
	}{
		{
			...
			validateFunc: func(t *testing.T, tempDir string, err error, ucc UpgradeClusterConfigGather) {
				...
				assert.Equal(t, "192.168.121.10", seedReconfig.NodeIPs[0])
...
func TestSeedReconfigurationDualStack(t *testing.T) {
	testcases := []struct {
		name                    string
...
func TestIPStackTypeDeterminationDualStack(t *testing.T) {
	testcases := []struct {
		name        string
		nodeIPs     []string
		description string
	...
	for _, tc := range testcases {
		t.Run(tc.name, func(t *testing.T) {
			...
			stackType := determineIPStackTypeDualStack(tc.nodeIPs)
...
func determineIPStackTypeDualStack(ips []string) string {
	hasIPv4 := false
	hasIPv6 := false
